...
public class whereInTheWorld {
    ...
    public static void main(String[] args) {
        ...
        while (scan.hasNextLine()) {
            ...
            if (Pattern.matches(
                    "^(\\d{1,3}(.\\d{1,6})*)(\\s*|\\s+)[NWES]((\\s*|\\s+),(\\s*|\\s+)|\\s+)\\d{1,3}(.\\d{1,6})*(\\s*|\\s+)[NWES](\\s.+|\\s+|)$",
                    inputEx)) {
            ...
            // standard form eg. +80.000 -120.00
                ...
                if (arrayIn.length == 8 && arrayIn[3].compareTo(arrayIn[7]) != 0) {
                    ...
                    if (Double.parseDouble(arrayIn[1]) >= 60.0 || Double.parseDouble(arrayIn[2]) >= 60.0
                            || Double.parseDouble(arrayIn[5]) >= 60.0 || Double.parseDouble(arrayIn[6]) >= 60.0){
                        System.out.println("Unable to process: " + input);
                        continue;
                    }
                ...
                if (arrayIn.length == 6 && arrayIn[2].compareTo(arrayIn[5]) != 0) {
                    // System.out.println(Arrays.toString(arrayIn));
                    if (Double.parseDouble(arrayIn[1]) >= 60.0 || Double.parseDouble(arrayIn[4]) >= 60.0){
                        System.out.println("Unable to process: " + input);
                        continue;
                    }
    ...
    public static String[] cleanToArray(String in) {
        String inEdit = in.replaceAll(",", " ").replaceAll("°", " ")
                .replaceAll("\'", " ").replaceAll("\"", " ").replaceAll("[dDmMs]", " ")
                .replaceAll("\\s+", " ");
